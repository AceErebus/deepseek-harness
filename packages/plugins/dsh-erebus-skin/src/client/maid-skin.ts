/**
 * Deep-sea maid atelier skin mount, ported from
 * dsh-deep-whale-main/maid-atelier (src/client/index.ts). The unified skin
 * manager decides activation (its axis), so the lane-marker / guest-band
 * machinery is removed; everything else — layered background, sidebar and
 * workspace chrome, titlebar brand, character stage, observers — is kept
 * verbatim, with the Cordis effect disposer turned into the returned cleanup.
 */
import {
...
} from './maid/background-art.generated.ts'
import {
...
} from './maid/chrome-art.generated.ts'
import {
  MAID_ATELIER_WORKSPACE_RIBBON,
  MAID_ATELIER_WORKSPACE_SHIELD,
} from './maid/workspace-art.generated.ts'
import { MAID_ATELIER_TITLEBAR_BRAND } from './maid/titlebar-brand.ts'
...
/** Minimal shape of the desktop-shell Window Controls Overlay (guarded by feature detection). */
...
function decorateWorkspaceTree(decoratedElements: Set<HTMLElement>): void {
  ...
  sidebar.querySelectorAll<HTMLElement>("[role='tree']").forEach((tree) => {
    ...
    const decorateGroup = (): void => {
      ...
      const lastSessionRow = sessionRows.at(-1)
      if (sessionRows[0]) sessionRows[0].dataset.maidSessionFirst = ''
      if (lastSessionRow) lastSessionRow.dataset.maidSessionLast = ''
...
/**
 * Mount the maid-atelier skin: layered background, sidebar/workspace chrome,
 * titlebar brand, character stage, and every skin-owned observer. Returns the
 * disposer that retracts each DOM/CSS write (ported from the original Cordis
 * effect body).
 */
export function mountMaidSkin(): () => void {
  ...
  const paintBackdropPlacement = (): void => {
  ...
  // Sidebar-width linkage writes into a dedicated <style> rule instead of the
  // body style: CSSOM edits produce no attribute mutations, so Chrome
  // autofill's MutationObserver never fires per frame and the curtain can
  // follow the sidebar width every frame without throttling.
  ...
  const styleSheet = widthSheet.sheet
  if (styleSheet === null) throw new Error('maid skin sidebar-width rule sheet is detached')
  styleSheet.insertRule('body { --maid-sidebar-width: 280px; --maid-sidebar-swag-height: 72.1px; --maid-sidebar-mascot-width: 229.6px; --maid-titlebar-height: 0px; }')
  ...
  const widthRule = styleSheet.cssRules[0] as CSSStyleRule
  ...
  const titlebarOverlay = (navigator as { windowControlsOverlay?: WindowControlsOverlayLike }).windowControlsOverlay
  ...
  const recoverRailSearchFocus = (event: MouseEvent): void => {
    const target = event.target instanceof Element
      ? event.target.closest<HTMLElement>("button[class*='searchButton']")
      : null
    const railSearch = target?.closest<HTMLElement>("[class*='search']") ?? null
  ...
  // The unified manager already decided this skin is the active one — apply
  // the guest visuals directly instead of watching an axis.
  body.dataset.dshMaidAtelier = ''
  syncBackdrop()
  paintBackdropPlacement()
